...
def main(config):
    ...
    if "returns_feat" in config["arch"]["args"]:
        model = config.init_obj(
            "arch", module_arch, allow_override=True, returns_feat=False
        )
    else:
...
def validation(data_loader, model, num_classes, device):
    ...
    eval_acc_mic_top1 = mic_acc_cal(
        preds[total_labels != -1], total_labels[total_labels != -1]
    )
    ...
    print(
        "{}, {}, {}".format(
            np.round(many_shot_acc * 100, decimals=2),
