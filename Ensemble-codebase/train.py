...
def learing_rate_scheduler(optimizer, config):
    if "type" in config._config["lr_scheduler"]:
        if config["lr_scheduler"]["type"] == "CustomLR":  # linear learning rate decay
            ...
            print(
                "Scheduler step1, step2, warmup_epoch, gamma:",
                (
                    lr_scheduler_args["step1"],
                    lr_scheduler_args["step2"],
                    lr_scheduler_args["warmup_epoch"],
                    gamma,
                ),
            )
...
def main(config):
    ...
    if hasattr(loss_class, "require_num_experts") and loss_class.require_num_experts:
        ...
        criterion = config.init_obj(
            "loss", module_loss, cls_num_list=data_loader.cls_num_list
        )
...
if __name__ == "__main__":
    ...
    options = [
        ...
        CustomArgs(
            ["--reduce_dimension"], type=int, target="arch;args;reduce_dimension"
        ),
        CustomArgs(
            ["--layer2_dimension"], type=int, target="arch;args;layer2_output_dim"
        ),
        CustomArgs(
            ["--layer3_dimension"], type=int, target="arch;args;layer3_output_dim"
        ),
        CustomArgs(
            ["--layer4_dimension"], type=int, target="arch;args;layer4_output_dim"
        ),
