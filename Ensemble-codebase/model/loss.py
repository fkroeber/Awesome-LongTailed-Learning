...
class FocalLoss(nn.Module):
    def __init__(self, cls_num_list=None, weight=None, gamma=0.0):
    ...
    def forward(self, output_logits, target):
        return focal_loss(
            F.cross_entropy(
                output_logits, target, reduction="none", weight=self.weight
            ),
            self.gamma,
        )
...
class RIDELoss(nn.Module):
    def __init__(
        ...
        if cls_num_list is None:
            ...
            per_cls_weights = (
                C * cls_num_list * reweight_factor + 1 - reweight_factor
            )  # Eq.3
            ...
            self.per_cls_weights_enabled_diversity = torch.tensor(
                per_cls_weights, dtype=torch.float, requires_grad=False
            ).cuda()  # 这个是logits时算diversity loss的weight
    ...
    def forward(self, output_logits, target, extra_info=None):
        ...
        for logits_item in extra_info["logits"]:
            ride_loss_logits = (
                output_logits if self.additional_diversity_factor == 0 else logits_item
            )
            if self.m_list is None:
