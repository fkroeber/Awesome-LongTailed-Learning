...
def load_state_dict(model, state_dict, no_ignore=False):
    ...
    for name, param in state_dict.items():
        if name not in own_state:  # ignore
            print(
                "Warning: {} ignored because it does not exist in state_dict".format(
                    name
                )
            )
            assert (
                not no_ignore
            ), "Ignoring param that does not exist in model's own state dict is not allowed."
            continue
        ...
        try:
            ...
            print(
                "Error in copying parameter {}, source shape: {}, destination shape: {}".format(
    ...
    if count != len(own_state):
        print(
            "Warning: Model has {} parameters, copied {} from state dict".format(
                len(own_state), count
            )
        )
    return count
