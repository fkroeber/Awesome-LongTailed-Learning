...
class model:
    def __init__(
        self, config, data, test=False, test_imb_ratio=1.0, test_reverse=False
    ):
        self.config = config
        ...
        if not test:
            self.tensorboard = SummaryWriter(
                log_dir=f"{self.training_opt['log_dir']}/tensorboard"
            )
        self.current_step = 0
        ...
        if not self.test_mode:
            ...
            if test_reverse:
                self.log_file = os.path.join(
                    ...
                    f"test-reverse_imb{test_imb_ratio}.txt",
                )
            else:
                self.log_file = os.path.join(
                    self.training_opt["log_dir"], f"test_imb{test_imb_ratio}.txt"
                )
            self.logger.log_cfg(self.config)
        ...
        for n, v in kargs.items():
            if hasattr(self, "tensorboard"):
                self.tensorboard.add_scalar(
                    tag=f"{split}/{n}", scalar_value=v, global_step=step
                )
    ...
    def init_models(self, optimizer=True):
        ...
        for key, val in networks_defs.items():
            ...
            self.model_optim_named_params.update(
                dict(self.networks[key].named_parameters())
            )
    ...
    def init_criterions(self):
        ...
        for key, val in criterion_defs.items():
            ...
            if val["optim_params"]:
                ...
                self.criterion_optimizer, self.criterion_optimizer_scheduler = (
    ...
    def init_optimizers(self, optim_params_dict):
        ...
        for key, val in networks_defs.items():
            # optimizer
            if (
                "optimizer" in self.training_opt
                and self.training_opt["optimizer"] == "adam"
            ):
            ...
            if scheduler_params["coslr"]:
                print(
                    "===> Module {} : Using coslr eta_min={}".format(
                        key, scheduler_params["endlr"]
                    )
                )
                self.model_scheduler_dict[key] = (
                    torch.optim.lr_scheduler.CosineAnnealingLR(
                        optimizer,
                        self.training_opt["num_epochs"],
                        eta_min=scheduler_params["endlr"],
                    )
                )
            elif scheduler_params["warmup"]:
                print("===> Module {} : Using warmup".format(key))
                self.model_scheduler_dict[key] = WarmupMultiStepLR(
                    optimizer,
                    scheduler_params["lr_step"],
    ...
    def show_current_lr(self):
        ...
        for key, val in self.model_optimizer_dict.items():
            ...
            print_str = [
                "=====> Current Learning Rate of model {} : {}".format(key, str(lr_set))
            ]
    ...
    def batch_forward(self, inputs, labels=None, feature_ext=False, phase="train"):
        ...
        if self.apply_pca:
            if phase == "train" and self.features.shape[0] > 0:
                ...
                self.features = (
                    torch.from_numpy(new_feat).float().to(self.features.device)
                )

        # update moving average
        if phase == "train":
            self.embed_mean = (
                self.mu * self.embed_mean
                + self.features.detach().mean(0).view(-1).cpu().numpy()
            )

        # If not just extracting features, calculate logits
        if not feature_ext:
            # cont_eval = 'continue_eval' in self.training_opt and self.training_opt['continue_eval'] and phase != 'train'
            self.logits, self.route_logits = self.networks["classifier"](
                self.features, labels, self.embed_mean
            )
    ...
    def batch_loss(self, labels):
        ...
        if "RouteWeightLoss" in self.criterions.keys():
            self.loss_route = self.criterions["RouteWeightLoss"](
                self.route_logits, labels
            )
            self.loss_route = (
                self.loss_route * self.criterion_weights["RouteWeightLoss"]
            )
    ...
    def train(self):
        ...
        print_write(
            ["Force shuffle in training??? --- ", self.do_shuffle], self.log_file
        )
        ...
        for epoch in range(1, end_epoch + 1):
            ...
            for key, model in self.networks.items():
                ...
                if self.config["networks"][key]["optim_params"]["lr"] == 0.0:
                    print_write(
                        [
                            "=====> module {} is set to eval due to 0.0 learning rate.".format(
                                key
                            )
                        ],
                        self.log_file,
            ...
            for step, (inputs, labels, indexes) in enumerate(self.data["train"]):
                ...
                with torch.set_grad_enabled(True):
                    ...
                    self.batch_backward(
                        print_grad=(step % self.training_opt["display_grad_step"] == 0)
                    )
                    ...
                    if step % self.training_opt["display_step"] == 0:
                        ...
                        records["sum_exp_logits"] = torch.mean(
                            self.sumexp_logits
                        ).item()
                        ...
                        self.write_summary(
                            split="train", step=self.current_step, **records
                        )
                        print_write(print_str, self.log_file)
    ...
    def store_logits(self, phase):
        ...
        for inputs, labels, paths in tqdm(self.data[phase]):
            ...
            with torch.set_grad_enabled(False):
                ...
                if hasattr(self.networks["classifier"].module, "thresholds"):
                    self.logits = (
                        self.route_logits
                        - self.networks["classifier"].module.thresholds
                    )
        ...
        np.save(
            os.path.join(self.training_opt["log_dir"], f"{phase}_total_logits"),
            self.total_logits.cpu().data.numpy(),
        )
        np.save(
            os.path.join(self.training_opt["log_dir"], f"{phase}_total_labels"),
            self.total_labels.cpu().data.numpy(),
        )
    ...
    def eval(self, phase="val", save_feat=False):
        ...
        for inputs, labels, paths in tqdm(self.data[phase]):
            ...
            with torch.set_grad_enabled(False):
                ...
                if save_feat:
                    self.saving_feature_with_label_update(
                        self.features, self.logits, labels
                    )
        ...
        self.eval_f_measure = F_measure(
            preds, self.total_labels, theta=self.training_opt["open_threshold"]
        )
    ...
    def load_model(self, model_dir=None):
        ...
        for key, model in self.networks.items():
            ...
            if (
                not self.test_mode
                and "Classifier" in self.config["networks"][key]["def_file"]
            ):
                if "tuning_memory" in self.config and self.config["tuning_memory"]:
                    ...
                    for param_name, param in self.networks[key].named_parameters():
                        ...
                        if "embed" in param_name:
                            ...
                            print(
                                "=====> Abandon Weight {} in {} from the checkpoints.".format(
                                    param_name, key
                                )
                            )
                        ...
                        print(
                            "=====> Tuning: {} | {}".format(
                                str(param.requires_grad).ljust(5, " "), param_name
                            )
                        )
                    print("=================================================")
                else:
            ...
            if all(
                [
                    weights[k].sum().item() == x[k].sum().item()
                    for k in weights
                    if k in x
                ]
            ):
                print(
                    "=====> All keys in weights have been loaded to the module {}".format(
                        key
                    )
                )
            else:
                print(
                    "=====> Error! Error! Error! Error! Loading failure in module {}".format(
                        key
                    )
                )
    ...
    def saving_feature_with_label_export(self):
        eval_features = {
            "features": torch.cat(self.saving_feature_container, dim=0).numpy(),
            "labels": torch.cat(self.saving_label_container, dim=0).numpy(),
            "logits": torch.cat(self.saving_logit_container, dim=0).numpy(),
        }

        eval_features_dir = os.path.join(
            self.training_opt["log_dir"], "eval_features_with_labels.pth"
        )
        torch.save(eval_features, eval_features_dir)
        print_write(
            ["=====> Features with labels are saved as {}".format(eval_features_dir)],
            self.log_file,
        )
    ...
    def calculate_thresholds(self, phase):
        ...
        if phase in self.data:
            ...
            thresholds = torch.logsumexp(store_logits, dim=0) - np.log(
                store_logits.size(0)
            )
