...
def main_worker(gpu, ngpus_per_node, config, logger, model_dir):
    ...
    criterion = LabelAwareSmoothing(
        cls_num_list=cls_num_list,
        smooth_head=config.smooth_head,
        smooth_tail=config.smooth_tail,
    ).cuda(config.gpu)

    optimizer = torch.optim.SGD(
        [{"params": classifier.parameters()}, {"params": lws_model.parameters()}],
        config.lr,
    ...
    for epoch in range(config.num_epochs):
        ...
        acc1, ece = validate(
            val_loader, model, classifier, lws_model, criterion, config, logger
        )
...
def train(
    ...
    progress = ProgressMeter(
        end_steps, [batch_time, losses, top1, top5], prefix="Epoch: [{}]".format(epoch)
    )
    ...
    for i, (images, target) in enumerate(train_loader):
        ...
        if config.mixup is True:
            images, targets_a, targets_b, lam = mixup_data(
                images, target, alpha=config.alpha
            )
            with torch.no_grad():
                feat = model(images)
