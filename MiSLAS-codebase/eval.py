...
def main():
    ...
    if config.multiprocessing_distributed:
        ...
        mp.spawn(
            main_worker, nprocs=ngpus_per_node, args=(ngpus_per_node, config, logger)
        )
    else:
...
def main_worker(gpu, ngpus_per_node, config, logger, model_dir):
    ...
    if config.dataset == "cifar10" or config.dataset == "cifar100":
        ...
        classifier = getattr(resnet_cifar, "Classifier")(
            feat_in=64, num_classes=config.num_classes
        )

    elif config.dataset == "imagenet" or config.dataset == "ina2018":
        model = getattr(resnet, config.backbone)()
        classifier = getattr(resnet, "Classifier")(
            feat_in=2048, num_classes=config.num_classes
        )

    elif config.dataset == "places":
        model = getattr(resnet_places, config.backbone)(pretrained=True)
        classifier = getattr(resnet_places, "Classifier")(
            feat_in=2048, num_classes=config.num_classes
        )
        block = getattr(resnet_places, "Bottleneck")(
            2048, 512, groups=1, base_width=64, dilation=1, norm_layer=nn.BatchNorm2d
        )
    ...
    if not torch.cuda.is_available():
        ...
        if config.gpu is not None:
            ...
            lws_model = torch.nn.parallel.DistributedDataParallel(
                lws_model, device_ids=[config.gpu]
            )

            if config.dataset == "places":
                block.cuda(config.gpu)
                block = torch.nn.parallel.DistributedDataParallel(
                    block, device_ids=[config.gpu]
                )
        else:
            model.cuda()
            classifier.cuda()
...
def validate(
    val_loader, model, classifier, lws_model, criterion, config, logger, block=None
):
    ...
    with torch.no_grad():
        ...
        for i, (images, target) in enumerate(val_loader):
            ...
            correct = correct + (target_one_hot + predict_one_hot == 2).sum(dim=0).to(
                torch.float
            )
        ...
        head_acc = (
            acc_classes[config.head_class_idx[0] : config.head_class_idx[1]].mean()
            * 100
        )
        med_acc = (
            acc_classes[config.med_class_idx[0] : config.med_class_idx[1]].mean() * 100
        )
        tail_acc = (
            acc_classes[config.tail_class_idx[0] : config.tail_class_idx[1]].mean()
            * 100
        )
        ...
        logger.info(
            "* ECE   {ece:.3f}%.".format(ece=cal["expected_calibration_error"] * 100)
        )
