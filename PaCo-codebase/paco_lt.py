...
def main_worker(gpu, ngpus_per_node, args):
    ...
    model = moco.builder.MoCo(
        (
            ...
            else getattr(resnet_imagenet, args.arch)
        ),
        args.moco_dim,
        args.moco_k,
        args.moco_m,
        ...
        if args.gpu is not None:
            ...
            if args.reload:
                ...
                for key in state_dict.keys():
                    print(key)
                    if (
                        key in state_dict_ssp.keys()
                        and state_dict[key].shape == state_dict_ssp[key].shape
                    ):
...
def validate(val_loader, train_loader, model, criterion, args):
    ...
    with torch.no_grad():
        ...
        many_acc_top1, median_acc_top1, low_acc_top1, cls_accs = shot_acc(
            preds, total_labels, train_loader, acc_per_cls=True
        )
        open(args.root_model + "/" + args.mark + ".log", "a+").write(
