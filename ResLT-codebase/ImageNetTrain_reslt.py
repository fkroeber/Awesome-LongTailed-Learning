...
def main_worker(gpu, ngpus_per_node, args):
    ...
    if args.pretrained:
        ...
        model = getattr(nets, args.arch)(
            dropout=args.dropout, after_1x1conv=args.after_1x1conv, gamma=args.gamma
        )
    ...
    data = getattr(dataset, args.dataset)(
        batch_size=args.batch_size, num_works=args.num_works, root=args.data_path
    )
...
def train(train_loader, model, criterion, optimizer, epoch, args):
    ...
    for i, (images, target) in enumerate(train_loader):
        ...
        I_loss = (
            crossEntropy(softmax, logitH, target, labelH, args.num_classes)
            + crossEntropy(softmax, logitM, target, labelM, args.num_classes)
            + crossEntropy(softmax, logitT, target, labelT, args.num_classes)
        ) / (labelH.sum() + labelM.sum() + labelT.sum())

        logit = logitH + logitM + logitT
        F_loss = (
            crossEntropy(softmax, logit, target, labelH, args.num_classes)
            / labelH.sum()
        )
