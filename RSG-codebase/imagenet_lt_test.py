...
parser.add_argument(
    "--best_checkpoint",
    type=str,
    default="checkpoint_rsg/ImageNet_LT_resnext50_32x4d/ckpt.best.pth.tar",
)
...
def shot_acc(
    ...
    for i in range(len(train_class_count)):
        ...
        if train_class_count[i] > many_shot_thr:
            many_shot.append(
                (class_correct[i].detach().cpu().numpy() / test_class_count[i])
            )
...
if __name__ == "__main__":
    ...
    print(
        "The overall accuracy: %.2f. The many shot accuracy: %.2f. The median shot accuracy: %.2f. The low shot accuracy: %.2f."
        % (
            overall_acc * 100,
            many_shot_overall * 100,
            median_shot_overall * 100,
            low_shot_overall * 100,
        )
    )
