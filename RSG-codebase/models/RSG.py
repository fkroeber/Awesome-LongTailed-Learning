...
class RSG(nn.Module):
    def __init__(
        self,
        n_center=3,
        feature_maps_shape=[32, 16, 16],
        num_classes=10,
        contrastive_module_dim=128,
        head_class_lists=[],
        ...
        self.contrastive_module = nn.Sequential(
            nn.Conv2d(
                self.C * 2, contrastive_module_dim, kernel_size=1, stride=1, padding=0
            ),
            nn.ReLU(inplace=True),
            nn.Conv2d(
                contrastive_module_dim,
                contrastive_module_dim,
                kernel_size=3,
                stride=1,
                padding=1,
    ...
    def compute_cesc_loss(self, feature_maps, c, gamma, target, epoch):
        ...
        if epoch <= self.epoch_thresh:
            ...
            pair_fea = (
                self.contrastive_module(feature_cat)
                .contiguous()
                .view(-1, self.contrastive_module_dim)
            )
            ...
            loss = torch.sum(
                torch.bmm(
                    ...
                    torch.pow(
                        (
                            feature_maps.unsqueeze(1).expand(
                                -1, c.size()[1], -1, -1, -1
                            )
                            - c
                        ),
                        2,
                    ).view(num, self.n_center, -1),
                )
            ) / (num) + F.cross_entropy(pair_pred, target_cat)
        else:
            loss = torch.sum(
                torch.bmm(
                    gamma,
    ...
    def compute_mv_loss(
        ...
        c_o = (
            torch.bmm(
                index, c.view(-1, self.n_center, self.H * self.W * self.C).double()
            )
            .view(ori_f.size())
            .cuda()
        )
        c_t = (
            torch.bmm(
                index_, c_.view(-1, self.n_center, self.H * self.W * self.C).double()
            )
            .view(ori_f.size())
            .cuda()
        )
        ...
        target_features_norm = F.normalize(
            target_features_vector.view(-1, self.C), dim=1
        )
        ...
        pair_fea = (
            self.contrastive_module(paired)
            .contiguous()
            .view(-1, self.contrastive_module_dim)
        )
        pair_pred = self.contrastive_fc(pair_fea)
        ...
        loss = F.cross_entropy(pair_pred, torch.zeros(num).long().cuda()) + (
            torch.sum(
                torch.abs(
                    torch.norm(var_map_t.view(-1, self.C), dim=1)
                    - torch.norm(var_map.view(-1, self.C), dim=1)
                )
            )
            + torch.sum(
                torch.abs(
                    target_features_norm * var_map_norm
                    - torch.ones(target_features_norm.size()).cuda()
                )
            )
        ) / (num)
    ...
    def forward(self, feature_maps, head_class_lists, target, epoch):
        ...
        index_head = torch.sum(
            (target_expand == head_class_lists_tensor).long(), dim=1
        ).cuda()
        ...
        maps_detach_fc = (
            torch.bmm(maps_detach_p.unsqueeze(1), linear).view(-1, self.n_center) + bias
        )
        gamma = self.softmax(maps_detach_fc)

        centers_ = (
            self.centers[target_select, :, :]
            .view(-1, self.n_center, maps_detach.size()[1])
            .unsqueeze(3)
            .unsqueeze(4)
            .repeat(1, 1, 1, maps_detach.size()[2], maps_detach.size()[3])
        )
            ...
            if num_head >= num_tail:
                ...
                for j in range(0, segment):
                    ...
                    feature_origin = maps_head[
                        j * num_tail : (j + 1) * num_tail, :, :, :
                    ]

                    maps_head_p = self.pooling(feature_origin).view(-1, self.C)
                    target_head_select = target_head[
                        j * num_tail : (j + 1) * num_tail
                    ].unsqueeze(1)
                    linear = self.linear[target_head_select, :, :].view(
                        -1, self.C, self.n_center
                    )
                    bias = self.bias[target_head_select, :].view(-1, self.n_center)

                    maps_head_fc = (
                        torch.bmm(maps_head_p.unsqueeze(1), linear).view(
                            -1, self.n_center
                        )
                        + bias
                    )
                    ...
                    center_origin = (
                        self.centers[target_head_select, :, :]
                        .view(-1, self.n_center, maps_detach.size()[1])
                        .unsqueeze(3)
                        .unsqueeze(4)
                        .repeat(1, 1, 1, maps_detach.size()[2], maps_detach.size()[3])
                    ...
                    linear_ = self.linear[target_tail_select, :, :].view(
                        -1, self.C, self.n_center
                    )

                    bias_ = self.bias[target_tail_select, :].view(-1, self.n_center)
                    maps_tail_fc = (
                        torch.bmm(maps_tail_p.unsqueeze(1), linear_).view(
                            -1, self.n_center
                        )
                        + bias_
                    )
                    ...
                    target_center = (
                        self.centers[target_tail_select, :, :]
                        .view(-1, self.n_center, maps_detach.size()[1])
                        .unsqueeze(3)
                        .unsqueeze(4)
                        .repeat(1, 1, 1, maps_detach.size()[2], maps_detach.size()[3])
                    )
                    loss_mv, feature_f = self.compute_mv_loss(
                        feature_origin,
                        center_origin,
                        target_center,
                        latent_2,
                ...
                for j in range(0, segment):
                    ...
                    maps_tail_p = self.pooling(latent_2).view(-1, self.C)
                    target_tail_select = target_tail[
                        j * num_head : (j + 1) * num_head
                    ].unsqueeze(1)
                    linear_ = self.linear[target_tail_select, :, :].view(
                        -1, self.C, self.n_center
                    )
                    ...
                    loss_mv_total += loss_mv
                    target = torch.cat(
                        (target, target_tail[j * num_head : (j + 1) * num_head]), dim=0
                    )
